...
Fixpoint interpret
  ...
  | Mlambda (xs, e) =>
    ...
    Func (x, locals, e)
  | Mapply (f, vs) =>
      match interpret locals env f, vs with
      | Func (x, locs, e), v :: vs => interpret (Ident.Map.add x (interpret locals env v) locs) env (Mapply (e, vs))
      | _, _ => fail "not a function"
      end
  ...
  | Mblock (tag, vals) =>
      Block (tag, Array_of_list (fail "empty array") (List.map (interpret locals env) vals))
  | Mfield (idx, b) =>
