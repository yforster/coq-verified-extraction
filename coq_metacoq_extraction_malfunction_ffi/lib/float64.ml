...
(** To be compatible with malfunction's representation of bool... *)

let is_nan f = not (is_nan f)
let is_infinity f = not (is_infinity f)
let is_neg_infinity f = not (is_neg_infinity f)
let sign f = not (sign f)
let eq x y = not (eq x y)
let lt x y = not (lt x y)
let le x y = not (le x y)
let equal x y = not (equal x y)
let is_float64 x = not (is_float64 x)
